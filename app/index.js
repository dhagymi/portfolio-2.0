...
class App {
    constructor() {
        ...
        console.log(
            `%cDeveloped & Designed by DHÁTeam ${String.fromCodePoint(
                0x0270c
            )}`,
    ...
    /* 	async createServiceWorker() {
		if ("serviceWorker" in navigator && !deviceDetection.isDesktop()) {
			try {
    ...
    onResize() {
        if (
            ...
            document.querySelector(".landscapeWarning").style.display = "flex";
        } else {
            document.querySelector(".landscapeWarning").style.display = "none";
        }
